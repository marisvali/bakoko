...
func TestPt_AddLen_ErrorTolerance(t *testing.T) {
	...
	// between 10 and 100 (exhaustive search).
	{
		...
		for x := 10; x < 100; x++ {
			for y := 10; y < 100; y++ {
				diff := AddLenGetDif(Pt{I(int64(x)), I(int64(y))}, extraLen)
...
func TestPt_SetLen(t *testing.T) {
	...
	targetLen := I(int64(math.Sqrt(float64(math.MaxInt64)) / 10))
	...
	// between 10 and 100 (exhaustive search).
	{
		...
		for x := 10; x < 100; x++ {
			for y := 10; y < 100; y++ {
				diff := SetLenGetDif(Pt{I(int64(x)), I(int64(y))}, targetLen)
				diffs = append(diffs, diff)
			}
		}
		//fmt.Println("maximum diff percentage-wise:", slices.Max(diffs))
